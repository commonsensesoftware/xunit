...
namespace Xunit.Sdk
{
    ...
    public class DisplayNameFormatter
    {
        ...
        public DisplayNameFormatter(TestMethodDisplay display)
        {
            ...
            if ((display & UseEscapeSequences) == UseEscapeSequences)
            {
                rule = new ReplaceEscapeSequenceRule() { Next = rule };
            }

            if ((display & ReplaceUnderscoreWithSpace) == ReplaceUnderscoreWithSpace)
            {
                rule = new ReplaceUnderscoreRule() { Next = rule };
            }

            if ((display & UseOperatorMonikers) == UseOperatorMonikers)
            {
                rule = new ReplaceOperatorMonikerRule() { Next = rule };
            }

            if ((display & ClassAndMethod) == ClassAndMethod)
            {
                if ((display & ReplacePeriodWithComma) == ReplacePeriodWithComma)
                {
